...
export class ResourceService {
  ...
  public async changeResourceStatus(id: number, status: ResourceStatus) {
    const ctx = { method: 'changeResourceStatus', entity: this._entity };
    this.logger.start(ctx);
    ...
    try {
      ...
      const record = await this.resourceRepository.save(resource);
      this.logger.success(ctx, 'updated');
      return ResponseFactory.success<ResourceResponseDto>(
        generateMessage('updated', this._entity, id),
